...
describe('ChartEffects', () => {
  ...
  const chartsBaseData: BaseChart[] = [
    {
  ...
  beforeEach(() => {
    when(dataServiceMock.getChart(anything())).thenCall(() => of(({ id: 'EURJPY', chart: chartsBaseData }) as Chart));
  });

  it('should be created', () => {
    expect(effects).toBeTruthy();
  });
  it('should call DataService when handling getChart action', done => {
    actions$ = of(getChart({ chart: 'EURJPY' }));

    effects.getChartsLoad$.subscribe(() => {
      verify(dataServiceMock.getChart(anything())).once();
      done();
    });
  });
});
